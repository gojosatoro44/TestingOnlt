...
# ================= UTILS =================
def load(p, d):
    """Load JSON data with proper error handling and data persistence"""
    with file_lock:
        try:
...
def admin_menu():
    return ReplyKeyboardMarkup(
        [["➕ Add Balance", "➖ Remove Balance"],
         ["📋 Add Verified IDs", "🗑️ View Verified IDs"],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid not in users:
        ...
        welcome_msg = (
            ...
            f"📊 Proofs Submitted: {users[uid]['proofs']}"
        )

    await update.message.reply_text(welcome_msg, reply_markup=menu())
...
async def add_ver_ids(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ Found {len(extracted_ids)} unique user ID(s):\n\n"
        f"{ids_preview}\n\n"
...
async def ver_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result_msg = f"✅ Successfully processed {len(extracted_ids)} ID(s)!\n\n"
    if added_count > 0:
        result_msg += f"➕ Added: {added_count} new ID(s)\n"
    if updated_count > 0:
        result_msg += f"🔄 Updated: {updated_count} existing ID(s)\n"
    result_msg += f"\n💰 Amount set: ₹{amount} for each ID\n"
    result_msg += f"📊 Total verified IDs now: {len(verified)}"
...
async def view_verified_ids(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for vid, amount in items:
        msg += f"🆔 {vid} → ₹{amount}\n"
    
    msg += f"\n📊 Total verified IDs: {len(verified)}"
